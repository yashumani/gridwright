...
| T06 | Delivered | `packages/bridge/src/sql.ts`, exported at `@yashumani/gridwright-bridge/sql` — read-only connector against a real SQLite fixture |
| T07 | Delivered | Explicit bindings, validated before computation |
| T08 | Delivered | Deterministic compile to a report definition |
| T09 | Delivered | Fill from view data, and `<Report>` in `@yashumani/gridwright-react` |
...
| A05 Unified numerical answer | **Passed** | One frozen snapshot behind chat, analytics and the report; `reconcile` compares the certified claim, the analysis and the bridge's total row. `scripts/verify-a05.mjs` drives the built surface in Chromium at desktop and phone: 18 checks, including that the answer quotes the figures the report draws and that the receipt is on the page rather than in a tooltip |
...
not, and what the difference depends on — the short version being that no
service has been contacted, and that is an access decision rather than an
engineering one.
